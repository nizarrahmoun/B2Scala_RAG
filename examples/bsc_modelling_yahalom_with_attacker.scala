...
/** Yahalom with a forwarder attacker. */
object BSC_modelling_Yahalom_withAttacker extends App {
  /////////////////////////////  DATA  /////////////////////////////
  ...
  lazy val Attacker: BSC_Agent = Agent {
    (
      get( message(alice, bob, Token("A1_A_B_NA")) ) *
      tell( message(alice, bob, Token("A1_A_B_NA")) ) *
      Attacker
    ) + (
      get( message(bob, serv, Token("B2_B_A_NB_NA")) ) *
      ...
      get( message(serv, alice, senc(pair(KAB, NB), kAS)) ) *
      tell( message(serv, alice, senc(pair(KAB, NB), kAS)) ) *
      Attacker
    ) + (
      get( message(serv, bob, senc(pair(KAB, pair(alice, NA)), kBS)) ) *
      tell( message(serv, bob, senc(pair(KAB, pair(alice, NA)), kBS)) ) *
      Attacker
    ) + (
      get( message(alice, bob, senc(NB, KAB)) ) *
      tell( message(alice, bob, senc(NB, KAB)) ) *
