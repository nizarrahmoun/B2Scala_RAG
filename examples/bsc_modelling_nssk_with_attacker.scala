...
/** Needham–Schroeder Symmetric (NSSK) with a forwarder attacker. */
object BSC_modelling_NSSK_withAttacker extends App {
  /////////////////////////////  DATA  /////////////////////////////
  ...
  lazy val Attacker: BSC_Agent = Agent {
    (
      ...
      get( message(kdc, alice, pair(senc(pair(KAB, NA), kAS), senc(pair(KAB, alice), kBS))) ) *
      tell( message(kdc, alice, pair(senc(pair(KAB, NA), kAS), senc(pair(KAB, alice), kBS))) ) *
      Attacker
    ) + (
      get( message(alice, bob, senc(pair(KAB, alice), kBS)) ) *
      tell( message(alice, bob, senc(pair(KAB, alice), kBS)) ) *
  ...
  /////////////////////////////  EXECUTION  /////////////////////////////
  val Protocol = Agent { Alice || (Bob || KDC_Agent) || Attacker }
