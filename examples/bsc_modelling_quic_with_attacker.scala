...
/** QUIC v1 with a simple Dolev–Yao forwarder attacker. */
object BSC_modelling_QUIC_withAttacker extends App {
  /////////////////////////////  DATA  /////////////////////////////
  ...
  lazy val Attacker: BSC_Agent = Agent {
    (
      get( message(client, server, initial(scid, dcid, tpC, client_hello(exp(g, x_eph), nc))) ) *
      tell( message(client, server, initial(scid, dcid, tpC, client_hello(exp(g, x_eph), nc))) ) *
      Attacker
    ) + (
      get( message(server, client, handshake(exp(g, y_eph), tpS, sign(sig_scope(exp(g,x_eph), exp(g,y_eph), scid, dcid, tpC, tpS), skS), pkS)) ) *
      tell( message(server, client, handshake(exp(g, y_eph), tpS, sign(sig_scope(exp(g,x_eph), exp(g,y_eph), scid, dcid, tpC, tpS), skS), pkS)) ) *
      Attacker
    ) + (
      get( message(client, server, finished(label_finished, prf(kdf(dh(x_eph, exp(g,y_eph)), label_quic), label_finished))) ) *
      tell( message(client, server, finished(label_finished, prf(kdf(dh(x_eph, exp(g,y_eph)), label_quic), label_finished))) ) *
      Attacker
    )
  }

  /////////////////////////////  LOGIC (BHM)  /////////////////////////////
  val End = bf( client_end_quic(pkS, kdf(dh(x_eph, exp(g, y_eph)), label_quic), scid, dcid) )
