...
/** Kerberos (AS+TGS+AP) with a forwarder attacker. */
object BSC_modelling_Kerberos_withAttacker extends App {
  /////////////////////////////  DATA  /////////////////////////////
  ...
  lazy val Attacker: BSC_Agent = Agent {
    (
      get( message(c, as, Token("AS_REQ")) ) *
      tell( message(c, as, Token("AS_REQ")) ) *
      Attacker
    ) + (
      get( message(as, c, pair(senc(pair(KTGS, NC), kC), senc(pair(KTGS, c), kTGS))) ) *
      tell( message(as, c, pair(senc(pair(KTGS, NC), kC), senc(pair(KTGS, c), kTGS))) ) *
      Attacker
    ) + (
      get( message(c, tgs, Token("TGS_REQ")) ) *
      tell( message(c, tgs, Token("TGS_REQ")) ) *
      Attacker
    ) + (
      get( message(tgs, c, pair(senc(KCS, KTGS), senc(pair(KCS, c), kS))) ) *
      ...
      get( message(c, s, pair(senc(pair(KCS, c), kS), senc(Token("authC"), KCS))) ) *
      tell( message(c, s, pair(senc(pair(KCS, c), kS), senc(Token("authC"), KCS))) ) *
      Attacker
    ) + (
      get( message(s, c, senc(Token("ok"), KCS)) ) *
      tell( message(s, c, senc(Token("ok"), KCS)) ) *
      Attacker
    )
  }
  ...
  /////////////////////////////  EXECUTION  /////////////////////////////
  val Protocol = Agent { Client || (ASrole || (TGSrole || Server)) || Attacker }
